...
Build a Go Telegram daemon that manages an independent `omp --mode rpc` process for each topic. Follow PLAN.md for design and acceptance criteria.
...
- Run gofmt after integration. Use `just` for all subsequent builds and test execution: `just` or `just build` compiles, `just test` builds and starts the foreground test service, and `just check` runs unit tests, race checks, and vet. Use `just test -check` for configuration validation. Do not bypass these recipes with direct Go build/test commands.
