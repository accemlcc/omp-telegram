...
在 Telegram topic 中使用 [Oh My Pi](https://github.com/can1357/oh-my-pi). 每个 topic 单独运行一个 omp 会话, 工作目录由你选择. 同一 topic 的消息依次处理, 不同 topic 可以并行工作.
...
- 已安装可正常运行的 omp 及其运行时, 如 Bun. 使用将要运行服务的同一个用户配置模型和认证.
- 能访问 Telegram 和模型服务.
- 一个 Telegram bot, 以及启用了 topic 的聊天. 支持群组 topic 和启用了 topic 的 bot 私聊, 不支持普通的无话题聊天.
...
2. 启用 topic 并创建一个要使用的话题. 群组中需要加入 bot 并允许其发送消息; 私聊需要开启 bot 的 topic/threaded mode. 本程序不会创建 topic.
...
就绪消息和 `/status` 都会显示 omp 原生 session ID, 恢复时建议使用完整 ID. `/resume` 列表切换要求 topic 空闲且没有排队消息. 普通文字不会自动创建实例, 不支持的斜杠命令也不会透传给 omp.
...
- 中断的任务不会自动重跑, 之前排队的普通消息会取消. 重发前先检查聊天和文件, 避免重复执行.
...
| `max_workers` | 同时运行的 topic 实例上限, 默认 4 |
| `queue_capacity` | 每个 topic 的等待消息上限, 默认 16 |
...
- 只授权可信用户. omp 拥有服务用户的文件访问权限和环境变量, 不同 topic 的独立会话不是文件系统或凭据沙箱.
...
| 现象 | 检查项 |
| --- | --- |
| bot 没有反应 | 两个白名单, topic 模式, 群隐私设置, 是否有其他 polling 服务或 webhook 占用 bot |
