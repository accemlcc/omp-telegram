...
The [release workflow](../.github/workflows/release.yaml) runs on `main` pushes, pull requests, and manual dispatch. All non-`main` branch changes must enter through a pull request. It builds/tests Linux amd64 and arm64 natively, with race checks on amd64. Every workflow from the repository publishes: a new source version on `main` creates a formal release without rewriting an existing version tag, while all other internal workflows update the `nightly` GitHub prerelease. Internal PRs publish the true head commit. External pull requests build but cannot publish. The workflow can update only the `nightly` tag or create a new formal version tag; it cannot overwrite a released version tag.
